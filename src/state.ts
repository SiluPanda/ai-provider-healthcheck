...
export class HealthStateMachine {
  ...
  private pendingState: HealthState | null = null;

  constructor(private readonly config: ResolvedMonitorConfig) {}
  ...
  evaluate(metrics: MetricsCollector, now?: number): StateEvaluation | null {
    ...
    return this.tryTransition(
      targetState,
      this.buildReason(targetState, stats.p95, errorRate),
      now
    );
  }
